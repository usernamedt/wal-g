...
// TODO : init tests
func (backup *Backup) getTarsToExtract(sentinelDto BackupSentinelDto, filesToUnwrap map[string]bool, skipUnusedTars bool) (tarsToExtract []ReaderMaker, pgControlKey string, err error) {
	...
	for _, tarName := range tarNames {
		...
		if skipUnusedTars && !shouldUnwrapTar(tarName, sentinelDto, filesToUnwrap) {
			continue
		}
